...
class AdminPermission
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        $permissionRules = [
            ...
            /*strtolower($controller .'@'. $requestMethod),20180529注释
        ...
        if(empty($auth) || !Admin::can($auth)){
            //ajax的返回json,其它返回首页
            if ($request->ajax()) {
                return new JsonResponse("抱歉，你没有对应的授权！", 403);
            } else {
                return Response::view(config('admin.role_auth_page'), ['role' => "抱歉，该菜单未授权访问！"], 403);
            }
        }
