...
<section class="content">
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="box box-info">
                ...
                <div class="box-body" >
                    <div class="row">
                        <form action="{{ url('game/matchcount/index') }}" method="get" id="search">
                        ...
                        <div class="form-group col-lg-2">
                            <select name="match_count_type" class="form-control"  id="moneyType">
                                <option
                                    value="">请选择比赛场
                                </option>
                                @foreach($active_type_arr as $active_type=>$active_name)
                                <option
                                    @if(Input::get('match_count_type') == $active_type )
                                    {{ 'selected' }}
                                    @endif
                    ...
                    <div class="form-group col-lg-2">
                        ...
                        <button type="button" style="margin-left:5px;" class="btn btn-default col-md-5" onclick="location.href ='{{$base_url}}/game/matchcount/index';">重置</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="box">
        ...
        <div class="box-header with-border">
            <h3 class="box-title">{{ $active_type_arr[Input::get('match_count_type')] }}</h3>
        </div>
        <div class="box-body">
            <table class="table table-bordered">
                ...
                <tr  >
                    <th>{{$date}}</th>
                    @foreach($type_arr as $k=>$v)
                    <th>{{isset($value[$k])?$value[$k]["pop_num"]:0}}|{{isset($value[$k])?$value[$k]["num"]:0}}</th>
                    @endforeach 
                </tr>  
                @endforeach 
            </table>
